export function createAnimationLoopManager(config = {}) {
  ...
  // charRenderer/charScene/charCamera/charSpot are optional — omit them for 2.5D sprite mode.
  const has3DChar = !!(charRenderer && charScene && charCamera && charSpot);
  ...
  // 3D character renderer — only used when has3DChar is true (GLB mode).
  // In 2.5D sprite mode this function is a no-op.
