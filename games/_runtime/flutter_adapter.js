...
(function () {
  ...
  function _getApiBase() {
    return localStorage.getItem('_flutter_game_api_base') || window.__AGORA_API_BASE_URL || _DEFAULT_API_BASE;
  }
  ...
  async function _fetchRtpRest(timeoutMs) {
    ...
    const cached = localStorage.getItem('_flutter_game_rtp');
    if (cached) {
      try {
    ...
    const gameId = localStorage.getItem('_flutter_game_id') || 'moon_dance';
    const jwt = localStorage.getItem('_flutter_game_jwt') || '';
  ...
  window.addEventListener('message', (e) => {
    ...
    if (type === 'HOST_INIT') {
      const { jwt, balance, demoMode, username, tgSafeAreaTop, rtp } = e.data;
